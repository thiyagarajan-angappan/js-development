...
// returns the count of each object in a provide an array, in this case an element
const getCount = function(elements, counter){
    let counts = [];
    for (let element of elements){
        let result = counter(element);
        let exists = counts.findIndex(f => f.result == result);
        
        if (exists == -1){
            counts.push({result, count: 1});
        }
        else{
            counts[exists].count++;
        }
...
// provided a char code return the script it belongs to
const findScript = function(charCode){
...
// returns the percentage of each script in a givent message
const scriptCounter = function(msg){
    let counts = getCount(msg, element => {
        let script = findScript(element.codePointAt(0));
        return script ? script.name : 'none';
    }).filter(({result}) => result != 'none');
    
    let total = counts.reduce((n,{count}) => n+count,0);
    
    counts = counts.map((element) => {
        return element.result + ' : ' + Math.round((element.count * 100)/total) + '%';
...
// find the direction in which most of the scripts in a given message are written
const dominantDirection = function(msg){
    let dir = getCount(msg, (element) => {
        let script = findScript(element.codePointAt(0));
        return script ? script.direction : 'nodir';
    }).filter(({result}) => result != 'nodir');
    
    return dir.length > 0 ? dir.reduce((a,b) => {
        return a.count > b.count ? a : b
    }).result : 'there is no direction';
};

console.log(scriptCounter('英国的狗说"woof", 俄罗斯的狗说"тяв"'));
